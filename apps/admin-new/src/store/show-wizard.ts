...
export const useShowWizardStore = create<WizardState>()(
  devtools(
    persist(
      (set, get) => ({
        // Initial state
...
// This store is specifically for the Show Wizard page management
// Most of the actual wizard state is managed by wizardStore.ts
interface ShowWizardPageState {
  // Any page-specific state needed
...
export const useShowWizardPageStore = create<ShowWizardPageState>()(
  devtools(
    persist(
      (set) => ({
        // Initial state
        ...
        // Reset the wizard by calling the main wizard store's reset
        resetWizard: () => {
          // Reset the main wizard store
          const { resetWizard: resetMainWizard } = useWizardStore.getState();
          resetMainWizard();
